...
public class TheJadeWaraxe extends SwordToolItem {	
    ...
    public void showAttack(Level level, int x, int y, AttackAnimMob mob, int attackHeight, InventoryItem item, int seed, PacketReader contentReader) {
        ...
        if (level.isClient()) {
			this.showAxeAttack(level, mob, seed, item);
        }
    }
    ...
    public InventoryItem onAttack(Level level, int x, int y, PlayerMob player, int attackHeight, InventoryItem item, PlayerInventorySlot slot, int animAttack, int seed, PacketReader contentReader) {
    	...
    	if (animAttack == 0) {
			...
			ToolItemMobAbilityEvent event = new ToolItemMobAbilityEvent(player, seed, item, x - player.getX(),
					y - player.getY() + attackHeight, animTime, animTime, new HashMap());			
    ...
    public InventoryItem onSettlerAttack(Level level, HumanMob mob, Mob target, int attackHeight, int seed, InventoryItem item) {
    	...
    	super.onSettlerAttack(level, mob, target, attackHeight, seed, item);
        return item;
    }
}
